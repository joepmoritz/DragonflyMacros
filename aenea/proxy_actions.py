    ...
    def _parse_spec(self, spec):
        ...
        for key in spec.split(','):
            ...
            if len(command_part) == 1:
                ...
                pause = int(pause_part[1]) if pause_part else None
                if not repeat:
                    continue
...
class ProxyText(ProxyBase, dragonfly.DynStrActionBase):
    def __init__(self, spec=None, static=False, pause=None):
        dragonfly.DynStrActionBase.__init__(self, spec, static)
...
class ProxyBringApp(ProxyBase, dragonfly.ActionBase):
    def __init__(self, window_title=None, process_title=None, process_name=None, window_not=None, start_action=None):
        dragonfly.ActionBase.__init__(self)
        self.window_title = window_title
        self.process_title = process_title
        self.process_name = process_name
        self.window_not = window_not
        self.start_action = start_action

    def _execute(self, data=None):
        print "Starting app process %s %s   window %s" % (self.process_name, self.process_title, self.window_title)
        result = aenea.communications.server.bring_app(
            window_title=self.window_title,
            process_title=self.process_title,
            process_name=self.process_name,
            window_not=self.window_not)
        
        if not result and self.start_action:
